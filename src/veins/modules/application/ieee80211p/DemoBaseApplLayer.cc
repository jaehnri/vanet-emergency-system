...
void DemoBaseApplLayer::initialize(int stage)
{
    ...
    if (stage == 0) {
        ...
        //kafe code starts
                sendAcdntEvt = new cMessage("acdnt evt", SEND_ACCIDENT_EVT);
                sendQueryEvt = new cMessage("query evt", SEND_QUERY_EVT);
                sendResponseEvt = new cMessage("response evt", SEND_RESPONSE_EVT);
                sendExpEvt = new cMessage("expiry evt", SEND_EXPIRY_EVT);
        //code end
        generatedBSMs = 0;
        generatedWSAs = 0;
        generatedWSMs = 0;
...
DemoBaseApplLayer::~DemoBaseApplLayer()
{
    ...
    //kafe code starts
        cancelAndDelete(sendAcdntEvt);
        cancelAndDelete(sendQueryEvt);
        cancelAndDelete(sendResponseEvt);
        cancelAndDelete(sendExpEvt);
    //code ends
